...
# save_rich_page + save_task_to_db (notion_tools) + add_job_application (career_tools)
_SAVE_TOOLS = [t for t in NOTION_TOOLS if t["name"] in ("save_rich_page", "save_task_to_db")]
_JOB_TOOL   = next(t for t in CAREER_TOOLS if t["name"] == "add_job_application")
NOTION_SAVE_TOOLS = _SAVE_TOOLS + [_JOB_TOOL]
