...
def _load_history_sample(max_messages: int = 200) -> list:
    ...
    try:
        # search_messages는 사용하지 않으므로 제거 (dead import 정리)
        from memory.history_db import get_recent_sessions, get_session_history
...
def get_profile_injection() -> str:
    """시스템 프롬프트에 주입할 사용자 맞춤 지시문 반환. 프로필이 없으면 빈 문자열 반환."""
