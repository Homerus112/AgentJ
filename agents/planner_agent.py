...
# 툴 이름 집합을 모듈 로드 시 한 번만 계산 (execute_tool 호출마다 재계산 방지)
_PLANNER_NAMES = {t["name"] for t in PLANNER_TOOLS}
_NOTION_NAMES  = {t["name"] for t in NOTION_TOOLS}
_GCAL_NAMES    = {t["name"] for t in GCAL_TOOLS}

def execute_tool(tool_name: str, tool_input: dict) -> str:
    if tool_name in _PLANNER_NAMES:
        return planner_execute(tool_name, tool_input)
    elif tool_name in _NOTION_NAMES:
        return notion_execute(tool_name, tool_input)
    elif tool_name in _GCAL_NAMES:
